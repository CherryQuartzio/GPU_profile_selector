...
const COMMAND_TO_SWITCH_GPU_PROFILE = "printf '%s\n' | pkexec envycontrol -s {profile} {choice1} {choice2}; gnome-session-quit --reboot";
...
export function switchHybrid(all_settings) {
    _execSwitch(GPU_PROFILE_HYBRID, all_settings.get_boolean("rtd3") ? "--rtd3 3" : "--rtd3 0", "");
}
...
export function switchNvidia(all_settings) {
    _execSwitch(GPU_PROFILE_NVIDIA, all_settings.get_boolean("force-composition-pipeline") ? "--force-comp" : "", 
        all_settings.get_boolean("coolbits") ? "--coolbits" : "");
}
